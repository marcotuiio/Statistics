...
t.test(depois, as.double(antes), alternative='two.side', mu=0, paired=T)
```

## Nesse caso, H0 é afirmar que não o treinamento não surtiu efeito positivo no grupo. Dado um p-valor de 0.1701, chegamos aa conclusão de que  a hipótese nula não deve ser rejeitada e portanto o treinamento não foi eficiente.
...
# Ex 5: Com o interesse em verificar se dois grupos são similares entre si em relação ao gasto médio com compras foi realizado um levantamento de dados. Com base nos resultados do conjunto de dados compras2.xlsx verifique se
existe diferença entre as variâncias e em seguida verifique se existe diferença entre os grupos. Para resolver o
exercício retire uma amostra de tamanho 15 para o grupo 1 e uma amostra de tamanho 15 para grupo 2, utilizando
o número de matrícula como semente.
...
## Realizando o teste das variâncias para garantir que são homogêneas chegamos a um p-valor de 0.4385. Dessa forma, concluimos que as variâncias são homogêneas e os dados apresentam distribuição normal (devido ao teste de Shapiro).

```{r}
t.test(gastos_g1, gastos_g2, var.equal=T)
```

## Em suma, pelo teste de hipótese com variâncias homogêneas, obtemos p-valor de 0.3291 em um intervalo de confiança de -570.42 - 19.79 e concluimos que não há evidências o suficiente para afirmar que os gastos dos grupos são diferentes, afinal o p-valor é superior a 0.05 não rejeito H0 e também pela presença de 0 na região crítica.
