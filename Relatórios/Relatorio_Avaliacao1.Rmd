...
# Erro superior a 100%, portanto algo está errado. Erros absurdos !!!
```
...
Analisando o box-plot podemos dizer que o primeiro quartil se encontra na faixa de gasto de 180, o segundo quartil (ou mediana) na faixa dos 200 e o terceiro quartil na faixa dos 240. Pode-se afirmar que a amostra se confirma relativamente simétrica, sem presença de elementos extremamente destoantes. Observa-se também que as maioria dos elementos se encontra na porção superior, visto que a caixa se encontra levemente deslocada para cima. A média e a mediana estarem tão próximas, cruz e linha central juntas, também é um bom indicativo da simetria no grupo.
...
yp = sorted[17]; yp
```
Assim, a promoção B deve focar nos produtos que os clientes gastando entre 173.43 e 215.16 consomem
...
yp = sorted[28]; yp 
```
Desse modo, a promoção C deve abrangir os produtos visados pelos clientes com gastos entre 215.16 e 259.72;
...
intervalos = c(min(sorted), min(sorted)+amp, min(sorted)+2*amp, min(sorted)+3*amp, min(sorted)+4*amp, min(sorted)+5*amp); intervalos  # Definindo vetor de intervalos
tabela = hist(sorted, plot=F, breaks=intervalos, right=F);

GASTOS = c("132.650|-159.026", "159.026|-185.402", "185.402|-211.778", "211.778|-238.154", "238.154|-264.530"); # Fazendo o nome das colunas a partir dos intervalos para exibir
...
## Variável quantitativa, e ja temos o desvio padrão, portanto calcula-se n0 por:
